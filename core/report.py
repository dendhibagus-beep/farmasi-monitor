...
def buat_laporan_bulanan_html(bulan_label: str, ringkasan_rows: list[dict], nama: str, jabatan: str, catatan: str,
                               tanda_tangan_data_url: str | None) -> str:
    """Laporan verifikasi BULANAN oleh Penanggung Jawab, termasuk rekap kepatuhan
    paraf harian per ruangan (berapa hari terisi dari total hari dalam bulan)."""
    baris_tabel = "\n".join(
        f"""<tr>
            <td>{r['Ruangan']}</td>
            <td>{r['Kepatuhan Paraf Harian']}</td>
            <td>{r['Min (°C)']}</td>
            <td>{r['Max (°C)']}</td>
            <td>{r['MKT (°C)']}</td>
...
<head>
<meta charset="utf-8">
<title>Laporan Verifikasi Bulanan — {bulan_label}</title>
<style>
...
<body>
    <h1>💊 Laporan Verifikasi Bulanan Suhu &amp; Kelembapan</h1>
    <div class="sub">Command Center Logistik &amp; Perbekalan Farmasi — Periode: <b>{bulan_label}</b><br>
    Ditinjau berdasarkan rekap paraf harian petugas logistik/duty farmasi sepanjang bulan.</div>

    <table>
        <thead>
            <tr>
                <th>Ruangan</th><th>Kepatuhan Paraf Harian</th><th>Min</th><th>Max</th>
    ...
    <p><b>Catatan Penanggung Jawab:</b><br>{catatan or '-'}</p>
    ...
    <div class="ttd-box">
        <div class="ttd-inner">
            ...
            <div>{jabatan}</div>
            <div style="color:#888; font-size:0.75rem;">Penanggung Jawab / Supervisor</div>
        </div>
    </div>
...
def buat_laporan_harian_html(tanggal, ringkasan_rows: list[dict], nama: str, jabatan: str, catatan: str,
                              tanda_tangan_data_url: str | None) -> str:
    """Laporan paraf HARIAN oleh petugas logistik / duty farmasi."""
    baris_tabel = "\n".join(
        f"""<tr>
            <td>{r['Ruangan']}</td>
...
<head>
<meta charset="utf-8">
<title>Paraf Harian — {tanggal}</title>
<style>
    body {{ font-family: Arial, Helvetica, sans-serif; color:#111; padding:2rem; max-width:800px; margin:auto; }}
    h1 {{ font-size:1.3rem; margin-bottom:0; }}
    .sub {{ color:#555; margin-top:0.2rem; margin-bottom:1.5rem; }}
...
<body>
    <h1>💊 Paraf Harian — Pengecekan Suhu &amp; Kelembapan</h1>
    ...
    <div class="footer">Dokumen ini dihasilkan otomatis oleh sistem pada {dibuat}.</div>
</body>
</html>"""
